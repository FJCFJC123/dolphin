...
void VertexManager::CreateDeviceObjects()
{
	...
	D3DCAPS8 DeviceCaps = D3D::GetCaps();
	...
	m_vertex_buffers = new LPDIRECT3DVERTEXBUFFER8[MAX_VBUFFER_COUNT];
	m_index_buffers = new LPDIRECT3DINDEXBUFFER8[MAX_VBUFFER_COUNT];
	...
	for (m_current_vertex_buffer = 0; m_current_vertex_buffer < MAX_VBUFFER_COUNT; m_current_vertex_buffer++)
	{
		// D3D8 CreateVertexBuffer/CreateIndexBuffer have no trailing pSharedHandle.
		if(FAILED( D3D::dev->CreateVertexBuffer( m_vertex_buffer_size,  D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY, 0, D3DPOOL_DEFAULT, &m_vertex_buffers[m_current_vertex_buffer] ) ) )
		{
			Fail = true;
			break;
		}
		if( FAILED( D3D::dev->CreateIndexBuffer( m_index_buffer_size * sizeof(u16),  D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY, D3DFMT_INDEX16, D3DPOOL_DEFAULT, &m_index_buffers[m_current_vertex_buffer] ) ) )
		{
			Fail = true;
			return;
		}
...
void VertexManager::DestroyDeviceObjects()
{
	...
	{
		if (m_vertex_buffers && m_vertex_buffers[i])
		{
			m_vertex_buffers[i]->Release();
			m_vertex_buffers[i] = NULL;
		}
		// Guard the index array too (may be NULL if CreateDeviceObjects bailed).
		if (m_index_buffers && m_index_buffers[i])
		{
...
void VertexManager::PrepareDrawBuffers(u32 stride)
{
	...
	if(IndexDataSize)
	{
		...
		if(FAILED(m_vertex_buffers[m_current_vertex_buffer]->Lock(m_vertex_buffer_cursor, datasize,(BYTE**)(&pVertices), LockMode)))
		{
			DestroyDeviceObjects();
			return;
		}
		...
		if(FAILED(m_index_buffers[m_current_index_buffer]->Lock(m_index_buffer_cursor * sizeof(u16), IndexDataSize * sizeof(u16), (BYTE**)(&pIndices), LockMode )))
		{
			DestroyDeviceObjects();
			return;
		}
...
void VertexManager::DrawVertexBuffer(int stride)
{
	...
	// D3D8: the base vertex index is a SetIndices parameter, not a draw argument.
	D3D::dev->SetIndices(m_index_buffers[m_current_index_buffer], basevertex);
	if (triangles > 0)
	{
