...
bool TextureCache::TCacheEntry::Save(const char filename[], unsigned int level)
{
	...
	return false;   // D3DX surface-to-file save is unavailable on Xbox
}
...
void TextureCache::TCacheEntry::FromRenderTarget(u32 dstAddr, unsigned int dstFormat,
	...
	const float *colmat)
{
	// TODO(task#15): EFB->texture copy — needs combined D3D8 SetRenderTarget(rt,ds),
	// the depth-matrix/color-copy shaders (task#14) and TextureConverter. Stubbed for
	// first-light (EFB copies aren't needed to drain the FIFO / un-hang the guest).
	(void)dstAddr; (void)dstFormat; (void)srcFormat; (void)srcRect;
	(void)isIntensity; (void)scaleByHalf; (void)cbufid; (void)colmat;
}

#if 0  // original DX9 EFB-copy body, restore + port under task #15
void TextureCache::TCacheEntry::FromRenderTarget_DX9(u32 dstAddr, unsigned int dstFormat,
	unsigned int srcFormat, const EFBRectangle& srcRect,
	bool isIntensity, bool scaleByHalf, unsigned int cbufid,
	const float *colmat)
{
...
#endif  // original DX9 FromRenderTarget
	...
	unsigned int scaled_tex_w, unsigned int scaled_tex_h)
{
	...
	// D3D8 CreateTexture has no trailing pSharedHandle argument.
